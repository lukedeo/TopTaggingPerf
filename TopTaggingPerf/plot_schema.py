...
def plot_roc(dictionary, schema, name = None, min_eff = 0, max_eff = 1, logscale = True, save_arr = False):
	if name is None:
		...
		savename = name
	roc = ROC_plotter(dictionary, min_eff = min_eff, max_eff = max_eff, linewidth=2.1, signal = schema['signal'], background = schema['background'], title = schema['title'], logscale = logscale, save_arr = save_arr)
